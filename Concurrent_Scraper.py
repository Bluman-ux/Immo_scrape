...
def extract_data_from_link(link):
    try:
        # Your code to extract data from the link goes here
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:  ###Adjust number of workers
        ###Submitting task for every link###
        futures = [executor.submit(extract_data_from_link, link_info["Url: "]) for link_info in urls_houses]
    ...
    extracted_data_csv_path = os.path.join(script_directory, subdirectory, f"Extracted_Data_{city_name}_{today}.csv")
